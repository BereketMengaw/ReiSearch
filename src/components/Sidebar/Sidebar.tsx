...
const navLinks = [
  { to: '/', label: 'Dashboard', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="6" fill="#f3f3f3"/><path d="M7 7h10v10H7z" fill="#222"/></svg>
  ) },
  ...
  { to: '/feed', label: 'Feed', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><rect x="4" y="5" width="16" height="14" rx="3" fill="#f3f3f3"/><rect x="7" y="8" width="10" height="2" rx="1" fill="#222"/><rect x="7" y="12" width="7" height="2" rx="1" fill="#222"/></svg>
  ) },
  { to: '/profile', label: 'Profile', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4" fill="#f3f3f3"/><circle cx="12" cy="8" r="3" fill="#222"/><rect x="6" y="15" width="12" height="5" rx="2.5" fill="#f3f3f3"/><rect x="8" y="16" width="8" height="2" rx="1" fill="#222"/></svg>
  ) },
  ...
  { to: '/link-crm', label: 'Link CRM', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><path d="M8 12a4 4 0 0 1 4-4h4a4 4 0 1 1 0 8h-4a4 4 0 1 1 0-8z" fill="#f3f3f3" stroke="#222" strokeWidth="1.5"/><path d="M8 12h8" stroke="#222" strokeWidth="1.5"/></svg>
  ) },
  { to: '/settings', label: 'Settings', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" fill="#f3f3f3" stroke="#222" strokeWidth="1.5"/><path d="M12 8v4l3 3" stroke="#222" strokeWidth="1.5" strokeLinecap="round"/></svg>
  ) },
  { to: '/logout', label: 'Log Out', icon: (
    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><rect x="4" y="4" width="16" height="16" rx="4" fill="#f3f3f3"/><path d="M15 12H9" stroke="#222" strokeWidth="1.5" strokeLinecap="round"/><path d="M12 9l3 3-3 3" stroke="#222" strokeWidth="1.5" strokeLinecap="round"/></svg>
  ) },
];
...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  useEffect(() => {
    // Simulate an async operation to get the onboarding percentage
    const simulateOnboarding = () => {
      ...
      const interval = setInterval(() => {
        ...
        if (currentPercent >= 90) {
          ...
          setOnboardingPercent(currentPercent);
        }
      }, 50);
      return () => clearInterval(interval);
    };

    const cleanup = simulateOnboarding();
    return () => cleanup();
  }, []);

  return (
    <>
      <aside className="hidden mt-10 lg:flex fixed mt-15 mb-5 left-3 top-10 bottom-0 z-30 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 w-60 h-screen rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 flex flex-col justify-between p-0 ml-2 max-h-[calc(100vh-5rem)] transition-colors duration-300">
        {/* Navigation */}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-800 p-3 flex flex-col gap-2 transition-colors duration-300">
              <div className="flex items-center justify-between">
                ...
                <span className="relative w-10 h-10 flex items-center justify-center">
                  ...
                  <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-blue-700">{onboardingPercent}%</span>
                </span>
              </div>
      ...
      <div className={`fixed inset-0 z-50 lg:hidden transition-all duration-300 ${sidebarOpen ? 'block' : 'pointer-events-none'}`} style={{zIndex: 1000}}>
        {/* Overlay */}
        <div
