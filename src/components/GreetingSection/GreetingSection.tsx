...
const GreetingSection: React.FC = () => (
  <section className="w-full max-w-3xl ml-0 mt-4 grid grid-cols-1 grid-rows-1 gap-3 mb-2 px-0">
    {/* Top left: Greeting, progress, profile, button (spans 2 columns) */}
    <div className="col-span-1 mt-2 row-span-1 bg-gray-200 dark:bg-gray-800 rounded-2xl shadow p-3 flex flex-col items-center gap-3 transition-colors duration-300">
      <div className="flex-1 flex flex-col justify-between h-full">
        <span className="text-gray-800 dark:text-gray-100 text-base font-semibold mb-1">Hey, Bereket</span>
        <span className="text-2xl font-bold text-gray-900 dark:text-white mb-0.5">73%</span>
        <div className="w-full h-2 rounded-full bg-blue-100">
          <div className="h-2 rounded-full bg-blue-500" style={{ width: '73%' }}></div>
        </div>
        <button className="flex items-center gap-1 px-2 bg-blue-500 text-white text-xs font-medium rounded-md hover:bg-blue-600 transition shadow w-fit mt-2">
      ...
      <img src={Bereket} alt="Profile" className="w-20 h-20 rounded-xl object-cover" />
    </div>

    {/* Top right: Public Info and Enable Notifications */}
    <div className="col-span-1 row-span-1 flex gap-2 min-w-0 mt-2">
    ...
    {/* bottom section */}
    <div className="col-span-1 row-span-1 bg-white dark:bg-gray-800 rounded-xl shadow p-2 flex gap-2 transition-colors duration-300">
      {/* Bottom left: Add Property */}
      <div className="flex-1 bg-purple-100 dark:bg-purple-900 rounded-xl shadow flex flex-col items-center justify-center p-2 min-w-0 transition-colors duration-300">
      ...
      <div className="flex-1 bg-blue-100 dark:bg-blue-900 rounded-xl shadow flex flex-col items-center justify-center p-2 min-w-0 border border-blue-100 dark:border-blue-900 transition-colors duration-300 relative overflow-hidden">
        {/* Faded logo background */}
        <img src={Logo} alt="ReiSearch Logo" className="absolute left-1/2 top-1/3 -translate-x-1/2 -translate-y-1/2 w-16 h-16 object-contain opacity-20 pointer-events-none select-none" />
        {/* Foreground content */}
        <span className="relative z-10 text-gray-500 dark:text-gray-300 top-1/4 text-xs font-semibold py-4 block text-center">Explore ReiDirectory</span>
      </div>
      {/* Bottom right: Explore Messaging */}
      <div className="flex-1 bg-green-100 dark:bg-green-900 rounded-xl shadow flex flex-col items-center justify-center p-2 min-w-0 transition-colors duration-300">
